...
async def get_table3(path: PathModel) -> str:
    ...
    str2 = datefunc(two_hour_ago) + two_hour_ago.strftime(str_for)
    str3 = datefunc(four_hour_ago) + four_hour_ago.strftime(str_for)
    str4 = datefunc(six_hour_ago) + six_hour_ago.strftime(str_for)
    str5 = datefunc(eight_hour_ago) + eight_hour_ago.strftime(str_for)
    str6 = datefunc(ten_hour_ago) + ten_hour_ago.strftime(str_for)
...
async def get_table4(path: PathModel) -> str:
    ...
    now_hour = datetime.now()
    two_hour_ago = now_hour - timedelta(hours=1)
    four_hour_ago = now_hour - timedelta(hours=2)
    six_hour_ago = now_hour - timedelta(hours=3)
    eight_hour_ago = now_hour - timedelta(hours=4)
    ten_hour_ago = now_hour - timedelta(hours=5)
    ...
    # 水位数据
    stations: List[Station] = await table4_data()
    await write_to_xlsx(path, columns_head, talbe_loc, stations)
    return path.dist